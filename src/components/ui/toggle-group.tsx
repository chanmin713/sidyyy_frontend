...
export function ToggleItem<T = string>({
  children,
  className = '',
}: ToggleItemProps<T>) {
  return (
    <div className={`flex-1 flex justify-center ${className}`}>{children}</div>
  );
}
...
export function ToggleBackground<T = string>({
  value,
  currentValue,
  hoveredValue,
  className = '',
}: ToggleBackgroundProps<T>) {
  const isActive = currentValue === value;
  ...
  const getPosition = () => {
    ...
    if (isHovered || isActive) {
      ...
      // 두 번째 항목 (following, popular 등)
        ...
        return 'left-[50%] w-[calc(50%-0.125rem)]';
      }
    }
    return '';
  };
